...
if st.session_state.get('analisis_berjalan', False):
    if nama_input and tanggal_input:
        ...
        # ==========================================
        # BLOK DONASI & CETAK PDF
        # ==========================================
        st.divider()
        st.header("📥 Cetak Manuskrip PDF Eksklusif")
        ...
        st.markdown(f"""
        <div style="background-color: #f8f9fa; border: 2px solid #28a745; border-radius: 10px; padding: 20px; text-align: center; margin-top: 20px; margin-bottom: 20px;">
            <p style="margin-bottom:5px; font-size: 1.15rem; color: #333;">Silakan kirim/transfer infaq sebesar <b>Rp 5.000</b> ke nomor DANA berikut:</p>
            <h2 style="margin:10px 0; color:#007BFF;">{NOMOR_DANA}</h2>
            <p style="font-size:1rem; color:#555; font-weight: 500;">Admin akan segera mengirimkan hasil cetak PDF karakter Juz Anda (atau dengan pasangan Anda) via WhatsApp.</p>
        </div>
        """, unsafe_allow_html=True)
        
        # --- UPDATE OTOMATISASI PESAN WHATSAPP ---
        tgl_user_str = tanggal_input.strftime("%d/%m/%Y")
        if analisis_pasangan_aktif:
            tgl_pasangan_str = tanggal_pasangan.strftime("%d/%m/%Y")
            teks_jenis_pdf = "Pasangan"
            detail_data = f"1. Anda: {nama_input} ({tgl_user_str})\n2. Pasangan: {nama_pasangan} ({tgl_pasangan_str})"
        else:
            teks_jenis_pdf = "Tunggal (Sendiri)"
            detail_data = f"Nama: {nama_input} ({tgl_user_str})"
            
        # Pesan mentah yang akan diformat untuk link WhatsApp
        pesan_mentah = f"Halo Admin, saya sudah transfer infaq Rp5.000 untuk cetak PDF Ngaji Diri ({teks_jenis_pdf}).\n\nMohon bantu cetakkan PDF dengan data berikut:\n{detail_data}\n\nTerima kasih."
        pesan_wa = urllib.parse.quote(pesan_mentah)
        ...
        # Tombol Download untuk Admin (Disembunyikan dengan rapi)
        with st.expander("🔑 Area Admin (Generate Laporan)"):
            kunci_input = st.text_input("Masukkan Kode Akses:", placeholder="Masukkan kode...")
            ...
            if kunci_input:
                if kunci_input.upper() == KODE_AKSES_PDF.upper():
                    ...
                    st.download_button(
                        label=f"📄 Unduh File {file_pdf_name}",
                        data=pdf_data,
